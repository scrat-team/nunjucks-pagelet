...
class Tag {
  ...
  escape(str) {
    const nunjucks = require('nunjucks');
    return nunjucks.lib.escape(String(str || ''));
  }
        ...
        Object.keys(item).forEach((key) => {
          ...
          if (key !== '__keywords') {
            if (Array.isArray(subItem)) {
              ...
              subItem = subItem.map(x => this.escape(x));
              collect.add(`${this.escape(key)}="${Array.from(new Set(subItem)).join(' ')}"`);
            } else if (typeof subItem === 'object') {
              // 对象, value为true的才输出, 譬如 style={a: true, b: false, c: bool} => style="a c"
              let trueKeys = Object.keys(subItem).filter(v => subItem[v]).map(x => this.escape(x));
              collect.add(`${this.escape(key)}="${trueKeys.join(' ')}"`);
            } else {
              // 字符串, 直接添加 key="value"
              collect.add(`${this.escape(key)}="${this.escape(subItem) || ''}"`);
            }
          }
        });
      }

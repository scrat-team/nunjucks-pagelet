...
class Resource {
   ...
   */
  getResById(id) {
    if (this.manifest && this.manifest.res) {
...
Resource.configure = function configure(opt) {
  // TODO: 移植到Config?
  // TODO: map结构在readme
  // TODO: map加载支持自定义, 支持直接传递, 直接在resource里面clean
  Resource.reset();
  const file = opt.file;
  const cache = opt.cache;
  const root = opt.root;
  assert.notEqual(file, undefined, '必须提供 file 参数, 作为`资源映射表`的文件路径');
  assert.notEqual(root, undefined, '必须提供 root 参数, 作为静态文件的根目录');

  logger = opt.logger || console;

  Resource.root = opt.root;
  Resource.prototype.comboURI = typeof opt.comboURI === 'function' ? opt.comboURI : Resource.prototype.comboURI;

  // 覆盖默认的helper实现
  Object.assign(helper, opt.helper);

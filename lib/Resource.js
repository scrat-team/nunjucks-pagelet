...
// auto inject sw register script when use PWA
...
class Resource {
  ...
  normalize(id, ext) {
    ...
    return id.replace(/^(?!(views|view|components|component)\/)/, 'component/');
  }
  ...
  // fix: 兼容comboPattern是绝对地址的情况
  getComboPattern() {
    const comboPattern = this.manifest.comboPattern || '/co??%s';

    return /^(https?:|)\/\//.test(comboPattern) ? comboPattern : this.domain + comboPattern;
  ...
  renderJs() {
    ...
    // 在<body>最后面插入sw register脚本
    if (this.manifest.usePWA && serviceWorkerRegistrationJS) {
    ...
    if (this._collect.css && this._collect.css.length) {
      this._collect.css.forEach(this.manifest.combo ? function(uri) {
        ...
        if (cache.hasOwnProperty(uri)) {
          ...
          const path = Resource.root + '/' + uri.replace('/public', '');
          try {
